...
function updateClaimsTable() {
    ...
    if (filteredClaims.length === 0) {
        tbody.innerHTML = '<tr><td colspan="9" class="empty-state">No claims match your filters</td></tr>';
...
function exportTableData() {
    // CSV Header
    let csv = 'Claim ID,Submitted Date,Customer Name,Mobile,Product,Issue,Status,Replacement Progress,Complete\n';

    // CSV Rows
    filteredClaims.forEach(claim => {
        const row = [
            claim.claim_id,
            (claim.submitted_date || '').replace(/,/g, ' '),
            (claim.customer_name || '').replace(/,/g, ' '),
            (claim.mobile_number || '').replace(/,/g, ' '),
            (claim.product || claim.model || '').replace(/,/g, ' '),
            (claim.issue || '').replace(/,/g, ' ').replace(/\n/g, ' '),
            (claim.status || '').replace(/,/g, ' '),
            calculateReplacementProgress(claim) + '%',
    ...
    link.setAttribute("download", "OSG_Claims_Export_" + new Date().toISOString().slice(0, 10) + ".csv");
