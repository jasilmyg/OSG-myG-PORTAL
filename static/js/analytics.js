...
function updateClaimsTable() {
    ...
    filteredClaims.forEach(claim => {
        ...
        // Build status cell - add tooltip for Rejected claims showing last follow-up
        const isRejected = (claim.status || '').toLowerCase() === 'rejected';
        const followUpText = (claim.follow_up_notes || '').trim();
        let statusCellHtml;
        if (isRejected && followUpText) {
            const safeFollowUp = followUpText.replace(/'/g, "&#39;").replace(/"/g, "&quot;");
            statusCellHtml = `<div class="rejected-tooltip-wrap" data-followup="${safeFollowUp}" onclick="event.stopPropagation()"><span class="status-pill ${statusClass}">${claim.status}</span></div>`;
        } else {
            statusCellHtml = `<span class="status-pill ${statusClass}">${claim.status}</span>`;
        }
    ...
    initRejectedTooltips();
    document.getElementById('showingCount').textContent = filteredClaims.length;
    document.getElementById('totalCount').textContent = allClaims.length;
}

// 🔴 Fixed-position tooltip for Rejected pills (escapes table overflow clipping)
function initRejectedTooltips() {
    let tip = document.getElementById('rj-followup-tip');
    if (!tip) {
        tip = document.createElement('div');
        tip.id = 'rj-followup-tip';
        tip.style.cssText = [
    ...
    document.querySelectorAll('.rejected-tooltip-wrap').forEach(wrap => {
        wrap.addEventListener('mouseenter', () => {
            const text = wrap.getAttribute('data-followup');
            if (!text) return;
            tip.textContent = text;
            tip.style.display = 'block';
            positionRejectedTooltip(tip, wrap);
        });
...
function positionRejectedTooltip(tip, wrap) {
...
function applyFilters() {
    ...
    filteredClaims = allClaims.filter(claim => {
        ...
        // Status (multi-select)
        const checkedStatuses = Array.from(document.querySelectorAll('.status-cb:checked')).map(cb => cb.value);
        if (checkedStatuses.length > 0) {
            const matchesStatus = checkedStatuses.some(sv => {
                if (sv === 'Replacement Completed') {
                    const s = (claim.status || '').toLowerCase();
                    return s.includes('replacement') && claim.complete;
                }
                return claim.status === sv;
            });
...
function clearAllFilters() {
    ...
    // Reset multi-select status checkboxes
...
// ===== Multi-Select Status Dropdown Controls =====
function toggleStatusDropdown(e) {
    e.stopPropagation();
    const btn = document.querySelector('.ms-dropdown-btn');
...
function toggleAllStatuses(cb) {
    // If "All Statuses" checked → uncheck all individual options
    document.querySelectorAll('.status-cb').forEach(c => c.checked = false);
    updateStatusLabel();
}

function updateStatusLabel() {
    const checked = Array.from(document.querySelectorAll('.status-cb:checked'));
    const allCb = document.getElementById('statusAll');
    ...
    if (checked.length === 0) {
        ...
        label.textContent = checked.length === 1
            ...
            : `${checked.length} statuses selected`;
    }
}
...
document.addEventListener('click', (e) => {
    ...
    if (dropdown && !dropdown.contains(e.target)) {
        ...
        document.querySelector('.ms-dropdown-btn')?.classList.remove('open');
    }
});
